...
describe Gramercy::Noun do

  describe "::construct" do
    it "returns a new proper noun" do
      expect(Gramercy::Noun.construct("Alice").is_proper).to be_truthy
    end

    it "returns a new common noun" do
      expect(Gramercy::Noun.construct("animal").is_proper).to be_falsey
    end
  end

  describe "::from" do
    before do
      @locust = Gramercy::Noun.create(base_form: 'locust')
    end

    it "finds a word by stem" do
      expect(Gramercy::Noun.from("locusts")).to eq @locust
    end

    it "returns a new noun if no match is found" do
      expect(Gramercy::Noun.from("vocals").base_form).to eq "vocals"
    end
  end

  describe ".base_form_or_synonym" do
    let(:noun) { Gramercy::Noun.new(
        base_form: "plague",
  ...
  describe ".plural" do
    let(:milk) { Gramercy::Noun.new(
        base_form: "milk",
        is_countable: false
      )
    }
    let(:tricycle) { Gramercy::Noun.new(
        base_form: "tricycle",
        is_countable: true
      )
    }
  ...
  describe ".possessive_singular" do
    let(:glasses) { Gramercy::Noun.new(base_form: 'glasses') }
    let(:djinn)   { Gramercy::Noun.new(base_form: 'djinn') }
  ...
  describe ".possessive_plural" do
    let(:general) { Gramercy::Noun.new(base_form: 'general') }
    let(:cactus)  { Gramercy::Noun.new(base_form: 'cactus', plural_form: 'cacti') }
